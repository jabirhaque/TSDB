...
This project is a simple **Time Series Database (TSDB)** implemented in C++ from scratch. It is designed to store sequential timestamped numeric data efficiently and provide a simple interface for reading and writing records. This application is only available on UNIX systems, since it is dependent on POSIX system functions.
...
- In-memory write buffers with periodic 5 ms fsync flushes, achieving sub-microsecond buffer append latency (avg 156 ns, p99 186 ns) while reducing syscall overhead
- Lightweight sparse index structure to support O(log n) range scans and timestamp specific queries
whilst controlling memory usage, delivering average read latency 10 µs, p99 14 µs
